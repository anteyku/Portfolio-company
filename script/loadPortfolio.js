let mesPortfolio = [  
  {
    img: `images/miniPost.webp`,
    title: `Площадка мини постов `,
    data: 0,
    github: `https://github.com/anteyku/minipost`,
    link: `https://github.com/anteyku/minipost`,
    description: `Данный проект публикует добавляемые вами посты без каких либо ограничений (в плане цензуры и свободы слова). При добавлении каждого поста - предоставляется удобны редактор текста и возможность загрузить картинку вместе с постом. На странице постов присутствует постраничная пагинация, имеется также страница с личным кабинетом где вы сможете изменить личные данные. Адаптирован под телефонные устройства, pixelPerfect верстка. Из за того что бесплатные хостинги не пре- доставляют бесплатную услугу хранения данных на своем сервере - проект не стоит на хостинге.`,
    slider: [
      `images/miniPost.webp`,
      `images/miniPost.webp`,
      `images/miniPost.webp`,
      `images/miniPost.webp`
    ],
    skills: [
      {
        image: `images/html.webp`,
        title: `HTML5`
      },
      {
        image: `images/css3.webp`,
        title: `CCS3`        
      },
      {
        image: `images/javascript.webp`,
        title: `JavaScript`        
      },
      {
        image: `images/less.webp`,
        title: `Less`        
      },
      {
        image: `images/node.webp`,
        title: `Node.js`        
      },
      {
        image: `images/mongo.webp`,
        title: `MongoDB`        
      },    
      {
        image: `images/boostrap.webp`,
        title: `Bootstrap`        
      }           
    ]        
  },      
  {
    img: `images/audio.webp`,
    title: `Хостинг музыки `,
    data: 1,
    github: `https://github.com/anteyku/audio-project`,
    link: `https://github.com/anteyku/audio-project`,
    description: `Данный сайт был разработан для понимание работы с audio и image на практики. Здесь можно добавить свою музыку вместе с картинкой, сама музыка при воспроизведение визуализируется в прямом Эфире в виде черточек и тем самым создают эффект проигрывания музыки . Сайт адаптивен под мобильные устройства, для адаптивной верстки использовалась технология Bootstrap. Pixel perfect верстка, дизайн и сама идея сайта была разработана также мною.`,
    slider: [
      `images/audioOne.webp`,
      `images/audioTwo.webp`,
      `images/audioThree.webp`,
      `images/audioFor.webp`
    ],
    skills: [
      {
        image: `images/html.webp`,
        title: `HTML5`
      },
      {
        image: `images/css3.webp`,
        title: `CCS3`        
      },
      {
        image: `images/javascript.webp`,
        title: `JavaScript`        
      },
      {
        image: `images/less.webp`,
        title: `Less`        
      },
      {
        image: `images/node.webp`,
        title: `Node.js`        
      },
      {
        image: `images/mongo.webp`,
        title: `MongoDB`        
      },    
      {
        image: `images/boostrap.webp`,
        title: `Bootstrap`        
      }           
    ]        
  },     
  {
    img: `images/pansionat.webp`,
    title: `Пансионат - Тепло родных`,
    data: 2,
    github: `https://github.com/anteyku`,
    link: `https://teploridnih.com/`,
    description: `Коммерческий проект, сайт представляет собой визитку пансионата "Тепло родных". На нем подробно написаны услуги которые предоставляет пансионат. Клиент написан на Vue 3, серверная сторона - фреймворк express.js на node.js. Имеется возможность отправлять формы на бэкенд, валидация содержимого форм на бекенде. Подключена гугл карта в подвале сайта. Из библиотек vue 3 - используется слайдер Swiper.js. Pixel Perfect резиновая верстка с адаптацией под мобильные устройства.`,
    slider: [
      `images/pansionatOne.webp`,
      `images/pansionatTwo.webp`,
      `images/pansionatThree.webp`,
      `images/pansionatFor.webp`
    ],
    skills: [
      {
        image: `images/html.webp`,
        title: `HTML5`
      },
      {
        image: `images/css3.webp`,
        title: `CCS3`        
      },
      {
        image: `images/javascript.webp`,
        title: `JavaScript`        
      },
      {
        image: `images/less.webp`,
        title: `Less`        
      },
      {
        image: `images/vue.webp`,
        title: `Vue 3`        
      },
      {
        image: `images/node.webp`,
        title: `Node.js`        
      },
      {
        image: `images/boostrap.webp`,
        title: `Bootstrap`        
      }                 
    ]        
  },   
  {
    img: `images/gameStarShip.webp`,
    title: `Игра - StarShip войны `,
    data: 3,
    github: `https://github.com/anteyku/starShip`,
    link: `https://anteyku.github.io/starShip/`,
    description: `Одиночная игра, где вам нужно сбивать ракетой астероиды выпуская по ним торпеды. С каждым уровнем выше - будет появляться больше астероидов. Всего ракета может выдержать четыре попадания. Задержка на стрельбу ракетами в одну секунду. Игра сделана для лучшего понимания разработки 2д игор. Написана на Canvas. Присутствуют звуки стрельбы, попадания, окончания игры и тд.`,
    slider: [
      `images/starShipOne.webp`,
      `images/starShipTwo.webp`,
      `images/starShipThree.webp`,
      `images/starShipFor.webp`
    ],
    skills: [
      {
        image: `images/html.webp`,
        title: `HTML5 + Canvas`
      },
      {
        image: `images/css3.webp`,
        title: `CCS3`        
      },
      {
        image: `images/javascript.webp`,
        title: `JavaScript`        
      },
      {
        image: `images/less.webp`,
        title: `Less`        
      }    
    ]        
  },    
  {
    img: `images/gameAvatar.webp`,
    title: `Игра - Препятствия Dino `,
    data: 4,
    github: `https://github.com/anteyku/gameDino`,
    link: `https://anteyku.github.io/gameDino/`,
    description: `Игра на подобии Google Dino. Нужно преодолевать препятствия на пути Динозаврика, набирать очки и соревноваться с друзями. Сама игра написана на технологии Canvas разработчиком Anteyku в целях практики и познания работы некоторых алгоритмов для дальнейшей более сложной разработке игор. В процессе разработки, я сталкивался с проблемой воспроизведения реалистичной анимации физики при падании и прыжке динозавра`,
    slider: [
      `images/gameOne.webp`,
      `images/gameDinoTwo.webp`,
      `images/gameDinoThree.webp`,
      `images/gameDinoFor.webp`
    ],
    skills: [
      {
        image: `images/html.webp`,
        title: `HTML5 + Canvas`
      },
      {
        image: `images/css3.webp`,
        title: `CCS3`        
      },
      {
        image: `images/javascript.webp`,
        title: `JavaScript`        
      },
      {
        image: `images/less.webp`,
        title: `Less`        
      }    
    ]        
  },    
  {
    img: `images/chat.webp`,
    title: `Анонимные чаты `,
    data: 5,
    github: `https://github.com/anteyku`,
    link: `https://anonim.onrender.com/auth`,
    description: `Проект анонимные чаты - сделан на технологии Socket Io. Это анонимные чаты которые не собирают каких либо данных кроме тех что идентифицируют ваш аккаунт. Каждому зарегистрированному пользователю доступно для создания 5 чатов, с возможностью удалить их. Также присутствует блок определяющий ТОП-5 чатов по количеству сообщений. Защита от меж сайтового скриптинга (XSS-уязвимостей). Препятствует обходу проверок ввода и DoS-атакам с помощью ошибки Uncaught TypeError в асинхронном коде, приводящей к сбою сервера. Препятствует внедрение в MongoDB вредоносного оператора. Например, злоумышленники могут отправить объект, содержащий оператор $ или включающий ., что способно изменить контекст операции с базой данных. Pixel Perfect резиновая верстка с адаптацией под мобильные устройства. `,
    slider: [
      `images/chatAuth.webp`,
      `images/chatIndex.webp`,
      `images/chatChat.webp`,
      `images/chatAdmin.webp`
    ],
    skills: [
      {
        image: `images/html.webp`,
        title: `HTML5`
      },
      {
        image: `images/css3.webp`,
        title: `CCS3`        
      },
      {
        image: `images/javascript.webp`,
        title: `JavaScript`        
      },
      {
        image: `images/node.webp`,
        title: `Node.js`        
      },
      {
        image: `images/mongo.webp`,
        title: `MongoDB`        
      },
      {
        image: `images/less.webp`,
        title: `Less`        
      }     
    ]        
  },    
  {
    img: `images/diary.webp`,
    title: `Зашифрованный электронный дневник`,
    data: 6,
    github: `https://github.com/anteyku`,
    link: `https://diary-mix1.onrender.com`,
    description: `Электронный дневник с авторизацией и регистрацией, шифрует все ваши записи в нем. Помимо этого дневник умеет распознавать количество дней в текущем месяце, а также дописывать оставшиеся количество. При создании дня вы можете выбрать одну из трех стилизаций отображаемых в дневника. Также можно удалить, редактировать, добавлять записи. Адаптивен под все телефонные устройства.`,
    slider: [
      `images/diary.webp`,
      `images/dineryAuthh.webp`,
      `images/diaryReg.webp`
    ],
    skills: [
      {
        image: `images/html.webp`,
        title: `HTML5`
      },
      {
        image: `images/css3.webp`,
        title: `CCS3`        
      },
      {
        image: `images/javascript.webp`,
        title: `JavaScript`        
      },
      {
        image: `images/node.webp`,
        title: `Node.js`        
      },
      {
        image: `images/mongo.webp`,
        title: `MongoDB`        
      },
      {
        image: `images/less.webp`,
        title: `Less`        
      }      
    ]
  },
  {
    img: `images/anime.webp`,
    title: `Видео хостинг - Аниме`,
    data: 7,
    github: `https://github.com/anteyku/anime-videos`,
    link: `https://anteyku.github.io/anime-videos/`,
    description: `Сверстанный аниме форум, предназначенный для просмотра аниме. Также реализованный мелкие детали типа слайдера и воспроизведения видео клипов на главной странице на клиентском javascript. Дизайн придуман и создан нами, на момент написания данного проекта у нас стояла цель - реализовать дизайн сложного и масштабного форума. Адаптивен под все телефонные устройства и планшеты.`,
    slider: [
      `images/anime.webp`,
      `images/animeTWO.webp`,
      `images/animeTHRRE.webp`,
      `images/animeFOR.webp`
    ],
    skills: [
      {
        image: `images/html.webp`,
        title: `HTML5`
      },
      {
        image: `images/css3.webp`,
        title: `CCS3`        
      },
      {
        image: `images/javascript.webp`,
        title: `JavaScript`        
      },
      {
        image: `images/less.webp`,
        title: `Less`        
      }      
    ]        
  },
  {
    img: `images/shopkey.webp`,
    title: `Магазин игровых ключей`,
    data: 8,
    github: `https://github.com/anteyku/game-keys-store`,
    link: `https://anteyku.github.io/game-keys-store/`,
    description: `Сверстанный макет - Магазина лицензионных ключей. Дизайн создан нами. Реализован скрипт слайдера, а также рандом рулетки игор. Создан с целю практики в версте масштабных проектов и создания дизайна. Имеет 4 уникальных страницы. Адаптивен под все телефонные устройства и планшеты.`,
    slider: [
      `images/game.webp`,
      `images/gameTwo.webp`,
      `images/gameThree.webp`,
      `images/gameFor.webp`
    ],
    skills: [
      {
        image: `images/html.webp`,
        title: `HTML5`
      },
      {
        image: `images/css3.webp`,
        title: `CCS3`        
      },
      {
        image: `images/javascript.webp`,
        title: `JavaScript`        
      },
      {
        image: `images/less.webp`,
        title: `Less`        
      }      
    ]        
  },
  {
    img: `images/christmasFOR.webp`,
    title: `Новогодний сайт подарков`,
    data: 9,
    github: `https://github.com/anteyku/Christmas-site`,
    link: `https://anteyku.github.io/Christmas-site/`,
    description: `Новогодний сайт подарков - идея в том что близкие покупают неизвестный для себя подарок родному человеку. Дизайн создан нами. Макет только сверстан. Адаптивен под все телефонные устройства и планшеты.`,
    slider: [
      `images/christmas.webp`,
      `images/christmasTwo.webp`,
      `images/christmasThree.webp`,
      `images/christmasFOR.webp`
    ],
    skills: [
      {
        image: `images/html.webp`,
        title: `HTML5`
      },
      {
        image: `images/css3.webp`,
        title: `CCS3`        
      },
      {
        image: `images/less.webp`,
        title: `Less`        
      }      
    ]        
  },
  {
    img: `images/csgoFive.webp`,
    title: `Магазин CS-GO кейсов `,
    data: 10,
    github: `https://github.com/anteyku/ase-store`,
    link: `https://anteyku.github.io/ase-store/`,
    description: `Чистая верстка магазина cs-go кейсов. Дизайн сделан нами. Макет сверстан за неделю. При его создании все картинки предметов в кейсах были взяты из открытого доступа в интернете. Сайт предназначался для открытия кейсов cs-go. Адаптивен под все телефонные устройства и планшеты.`,
    slider: [
      `images/csgoTwo.webp`,
      `images/csgoThree.webp`,
      `images/csgoFor.webp`,
      `images/csgoFive.webp`
    ],
    skills: [
      {
        image: `images/html.webp`,
        title: `HTML5`
      },
      {
        image: `images/css3.webp`,
        title: `CCS3`        
      },
      {
        image: `images/less.webp`,
        title: `Less`        
      }      
    ]        
  },
  {
    img: `images/youtubeTwo.webp`,
    title: `Сайт визитка для ютубера`,
    data: 11,
    github: `https://github.com/anteyku/horror`,
    link: `https://anteyku.github.io/horror/`,
    description: `Сайт сверстан для рассказчика страшных историй Дмитрий Ray. Дизайн создан нами. Сам канал https://www.youtube.com/channel/UCYcygnLe4cCuo2Zx0jtkcyg. Адаптивен под все телефонные устройства и планшеты.`,
    slider: [
      `images/youtubeTwo.webp`,
      `images/youtubeThree.webp`,
      `images/youtubeFor.webp`,
      `images/youtubeFive.webp`
    ],
    skills: [
      {
        image: `images/html.webp`,
        title: `HTML5`
      },
      {
        image: `images/css3.webp`,
        title: `CCS3`        
      },
      {
        image: `images/less.webp`,
        title: `Less`        
      }      
    ]        
  }             
]



for(let i = 0; i < mesPortfolio.length; i++){
  createElem(mesPortfolio[i].img, mesPortfolio[i].title, mesPortfolio[i].data, i, true);
}


function createElem(img, name, data, num, first){
  
  let portfolio = document.querySelector(`.portfolio__project`);

  let block = document.createElement(`div`);
  block.classList.add(`portfolio__project__block`);
  block.dataset.num = data;
  
  let blockImage = document.createElement(`div`);
  blockImage.classList.add(`portfolio__project__block__image`);

  let blockImageIMG = document.createElement(`img`);
  blockImageIMG.dataset.src = img;
  blockImageIMG.classList.add(`lazyload`);

 let blockTitle = document.createElement(`div`);
 blockTitle.classList.add(`portfolio__project__block__title`);
 blockTitle.innerHTML = name;

 blockImage.appendChild(blockImageIMG);
 block.appendChild(blockImage);
 block.appendChild(blockTitle);

 if(num > 2 && first == true){
  block.style.display = `none`;
 }

 portfolio.appendChild(block);
}



// Закрытия портфолио
for(let elem of document.querySelectorAll(`.modulePortfolio__close`)){
  elem.addEventListener(`click`, (event)=>{
    event.composedPath()[2].style.display = `none`;
    event.composedPath()[2].style.overflowY = `hidden`;
    // Удаления всех картинок в поле "СКРИНШОТЫ" в модульном оккне
    for(let elem of document.querySelectorAll(`.modulePortfolio__screen__slider__img`)){
      document.querySelector(`.modulePortfolio__screen__slider`).removeChild(elem);
    }

    // Удаления всех скилов в поле "ПРИМЕНЯЕМЫЕ НАВЫКИ" в модульном окне
    for(let elem of document.querySelectorAll(`.skil`)){
      document.querySelector(`.skill`).removeChild(elem);
    }    

    document.querySelector(`html`).style.overflowY = `scroll`;
    
  })
}


// Открытия модульного окна при нажатии на проект
let modalPort = document.querySelector(`.modulePortfolio`);
let modalTitle = document.querySelector(`.modulePortfolio__top__title`);
let gitHub = document.querySelector(`.modulePortfolio__top__github`);
let linkSite = document.querySelector(`.modulePortfolio__top__button`);
let description = document.querySelector(`.modulePortfolio__description__about`);
let sliders = document.querySelector(`.modulePortfolio__screen__slider`);
let blockSkill = document.querySelector(`.skill`);

for(let elem of document.querySelectorAll(`.portfolio__project__block`)){
  elem.addEventListener(`click`, (event)=>{
    document.querySelector(`html`).style.overflow = `hidden`;
    // Делаим видимым модульное окно
    modalPort.style.display = `block`;
    // Добавляем в окно scroll
    modalPort.style.overflowY = `scroll`;
    // Ставим всю инфу из нашего массива mesPortfolio
    modalTitle.innerHTML = mesPortfolio[elem.dataset.num].title;
    gitHub.href = mesPortfolio[elem.dataset.num].github;
    linkSite.href = mesPortfolio[elem.dataset.num].link;
    description.innerHTML = mesPortfolio[elem.dataset.num].description;

    
    // Добавляем скриншоты сайта в модульное окно
    for(let i = 0; i <  mesPortfolio[elem.dataset.num].slider.length; i++){
      let layzeLoad = document.createElement(`div`);
      layzeLoad.classList.add(`modulePortfolio__screen__slider__img`);
      let layzeLoadIMG = document.createElement(`img`);
      layzeLoadIMG.classList.add(`lazyload`);
      layzeLoadIMG.dataset.src = mesPortfolio[elem.dataset.num].slider[i];
      layzeLoad.appendChild(layzeLoadIMG);
      sliders.appendChild(layzeLoad);
    }

    // Добавления всех скилов
    for(let i = 0; i <  mesPortfolio[elem.dataset.num].skills.length; i++){
      let Kenta = document.createElement(`div`);
      Kenta.classList.add(`moduleAbout__skills__langues__image`);
      let KentaIMG = document.createElement(`img`);
      KentaIMG.classList.add(`lazyload`);
      KentaIMG.dataset.src = mesPortfolio[elem.dataset.num].skills[i].image;  
      let skillText = document.createElement(`div`);
      skillText.classList.add(`moduleAbout__skills__langues__text`);
      skillText.innerHTML = mesPortfolio[elem.dataset.num].skills[i].title;
      let skillsL = document.createElement(`div`);
      skillsL.classList.add(`moduleAbout__skills__langues`);
      skillsL.classList.add(`skil`);
      Kenta.appendChild(KentaIMG);
      skillsL.appendChild(Kenta);
      skillsL.appendChild(skillText);
      blockSkill.appendChild(skillsL);
      
    }  


  })
}




// Нажатия на кнопку "Загрузить еще"
let loadY = document.querySelector(`.loadYet`);
let SldLoad = document.querySelector(`.portfolio`);
loadY.addEventListener(`click`, ()=>{
  for(let elem of document.querySelectorAll(`.portfolio__project__block`)){
    console.log(elem.dataset);
    if(elem.dataset.num > 2){
      elem.style.display = `block`;
    }
  }

  SldLoad.removeChild(loadY);
})













